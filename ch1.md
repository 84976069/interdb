## 第一章  数据库集群、数据库和表{docsify-ignore} 
...
![Fig. 1.1. Logical structure of a database cluster.](https://github.com/84976069/interdb/blob/master/imgs/ch1/fig-1-01.png?raw=true)
...
![Fig. 1.2. An example of database cluster.](https://github.com/84976069/interdb/blob/master/imgs/ch1/fig-1-02.png?raw=true)
...
仔细查看数据库子目录，您会发现每个表都有两个相关的文件，分别以'fsm'和'vm'作为后缀。这些被称为**空闲空间映射表(free space map)**和**可见性映射表(visibility map)**，分别在表文件中存储每个page页的空闲空间容量和可见性的信息(更详细的信息见[5.3.4节](ch5.md)和[6.2节](ch6.md))。索引只有空闲空间映射，没有可见性映射。
...
![Fig. 1.3. A Tablespace in the Database Cluster.](https://github.com/84976069/interdb/blob/master/imgs/ch1/fig-1-03.png?raw=true)
...
![Fig. 1.4. Page layout of a heap table file.](https://github.com/84976069/interdb/blob/master/imgs/ch1/fig-1-04.png?raw=true)

表中的page页包含三种数据描述如下：

1. **元组(heap tuples)(s)** – 一个tuple就是一条数据记录。它们从page页底部开始依次堆叠。[第5.2节](ch5#502)和[第9章](ch9.md)描述了tuple的内部结构，因为需要同时了解PostgreSQL中的并发控制和WAL。

2. **行指针(line pointer)(s)** – 一个行指针长度为4个字节，并且包含指向每个元组的指针。它也被称为**条目指针(item pointer)**。

   行指针形成一个简单的数组，它扮演着元组索引的角色。每个索引都从1开始顺序编号，称为**偏移号(offset number)**。当一个新的元组被添加到page页时，一个新的行指针被推到数组上以指向新的行。

3. **头数据(header data)** – 由 [PageHeaderData](javascript:void(0)) 结构定义的数据分配在page页的开头。它是24个字节长，并包含有关page页的一般信息。下面描述该结构的主要变量。

   - *pd_lsn* – 该变量存储由该page页的最后一次更改写入的XLOG记录的LSN。它是一个8字节的无符号整数，与WAL机制有关。细节在[第9章](ch9.md)中描述。
   - *pd_checksum* – 该变量存储此page页的校验和值。(请注意，此版本在9.3版本或更高版本中受支持;在较早版本中，此部分存储了page页的时间轴ID。)
   - *pd_lower, pd_upper* – pd_lower指向行指针的末尾，pd_upper指向最新的元组的开头。
   - *pd_special* – 这个变量是用于索引的。在表table的page页中，它指向page页的结尾。(在索引内的page页中，它指向特殊空间的开始，它是仅由索引保存的数据区域，并且根据诸如B-tree，GiST，GiN等索引类型的种类包含特定数据)


行指针末尾和最新元组开头之间的空白空间称为**空闲空间(free space)**或**空洞(hole)**。

为了标识表中的元组，在内部使用**元组标识符(tuple identifier)(TID)**。一个TID包含一对值：包含该元组的page页的块号，以及指向该元组的行指针的偏移号。其典型的应用实例是索引。更多细节见第1.4.2节。
...
另外，使用称为**TOAST**(The Oversized-Attribute Storage Technique)的方法存储和管理大小大于约2KB(约为8KB的1/4)的元组。有关详细信息，请参阅 [PostgreSQL 文档](http://www.postgresql.org/docs/current/static/storage-toast.html)。

## 1.4. 写表和读表的方法

在本章的最后，描述了写入和读取元组tuple的方法。

### 1.4.1. 写元组

假设一个表由一个只包含一个元组的page页组成。该page页的pd_lower指向第一行指针，并且行指针和pd_upper都指向第一个元组。参见图1.5(a)。

当第二个元组被插入时，它被放置在第一个元组之后。第二行指针被推到第一行指针后，并指向第二个元组。pd_lower更改为指向第二行指针，pd_upper更改为第二个元组。参见图1.5(b)。此page页中的其他头数据(例如，pd_lsn，pg_checksum，pg_flag)也被重写为适当的值; 更多细节在[第5.3节](ch5.md)和[第9章](ch9.md)中描述。

图 1.5. 写tuple

![Fig. 1.5. Writing of a heap tuple.](https://github.com/84976069/interdb/blob/master/imgs/ch1/fig-1-05.png?raw=true)
...
- **B树索引扫描(B-tree index scan)** – 索引文件包含索引元组，每个索引元组由索引键和指向目标元组的TID组成。如果找到了正在查找的key值的索引元组，PostgreSQL使用获取的TID值读取所需的元组。(关于在B树索引中查找索引元组的方法的描述，在这里不进行讲解，因为它很常见，并且这里篇幅有限，详情参见相关资料)。例如，在图1.6(b)中，TID 获得的索引元组的值是‘(block = 7, Offset = 2)’。这意味着目标元组是表中第7页的第2个元组，因此PostgreSQL可以读取所需的元组，而不必在page页中进行不必要的扫描。

图. 1.6.  顺序扫描和索引扫描

![Fig. 1.6. Sequential scan and index scan.](https://github.com/84976069/interdb/blob/master/imgs/ch1/fig-1-06.png?raw=true)
...
>```
>
> Index-Only-Scan将在[第7章](ch7.md)中详细介绍。
